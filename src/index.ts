...
async function getWalletBalance(
  ...
  try {
    ...
    logger.info(
      `Balance: ${balance} sats (${(balance / 100000000).toFixed(8)} BTC)`,
    );
...
async function fetchAllTransactions(
  ...
  try {
    ...
    const mempoolRes = await axios.get(
      `${esploraApiUrl}/address/${address}/txs/mempool`,
    );
    const mempool: any[] = Array.isArray(mempoolRes.data)
      ? mempoolRes.data
      : [];
    ...
    const firstPageRes = await axios.get(
      `${esploraApiUrl}/address/${address}/txs`,
    );
    ...
    logger.info(
      `Total confirmed txs: ${confirmed.length}, mempool txs: ${mempool.length}`,
    );
...
async function sendTransaction(
  ...
  try {
    ...
    for (const u of utxos) {
      ...
      logger.info(
        `Selection step: inputs=${selectedUtxos.length}, estVBytes=${estVBytes}, estFee=${fee}, inputValue=${inputValue}`,
      );
    ...
    logger.info(
      `Fee estimate: estVBytes=${estVBytesInitial}, feeRate=${feeRate}, estimatedFee=${estimatedFee}, inputValue=${inputValue}, amount=${amount}, provisionalChange=${provisionalChange}`,
    );
    ...
    if (provisionalChange > 546) {
      psbtMeasure.addOutput({
        address: senderAddress,
        value: provisionalChange,
      });
    }
    ...
    logger.info(
      `Fee exact: vsize=${exactVSize}, feeRate=${feeRate}, exactFee=${exactFee}, exactChange=${exactChange}`,
    );
    ...
    if (exactChange > 546) {
      ...
      logger.info(
        `Change output added: ${exactChange} sats to ${senderAddress}`,
      );
    } else {
      logger.info(
        `No change output (<= dust threshold). Extra sats go to fee.`,
      );
    }
...
fetchAllTransactions(
  ...
  "tb1q6hks4hg2fv80g0rpk9732uem4z3sytp7gv9dea",
);
